...
# Show the most common non-transparent pixel colors
non_transparent = data[:,:,3] > 0
nt_pixels = data[non_transparent]
print(f"Non-transparent pixels: {nt_pixels.shape[0]}")

# Check what RGB values exist in non-transparent pixels
...
# More aggressive removal - lower threshold to 200 and also handle anti-aliased edges
# Remove all light/white-ish pixels (not green)
# The green logo color is around RGB(32, 178, 129) based on the image
# So anything that's NOT greenish and is light should be made transparent

# Approach: remove anything where all RGB channels are above 200
threshold = 200
light_mask = (data[:,:,0] > threshold) & (data[:,:,1] > threshold) & (data[:,:,2] > threshold)
data[light_mask, 3] = 0

# Also handle any near-white with slight variations
# Pixels where the average RGB > 220 and the difference between channels is small (grayish/whitish)
